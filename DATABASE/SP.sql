...
BEGIN
	SELECT MAMH
	FROM (SELECT MALTC, MAMH FROM LOPTINCHI WHERE NIENKHOA = @NK AND HOCKY = @HK) LTC,
		(SELECT MALTC FROM DANGKY WHERE MASV = @MASV AND (DIEM_CC IS NOT NULL OR DIEM_GK IS NOT NULL OR DIEM_CK IS NOT NULL) 
		AND HUYDANGKY = 'FALSE') DK
	WHERE LTC.MALTC = DK.MALTC
	-- GROUP BY MAMH
END
GO
