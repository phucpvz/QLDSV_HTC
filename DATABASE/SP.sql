...
AS
BEGIN
	SELECT MASV, HOTENSV = HO + ' ' + TEN, SV.MALOP, MAKHOA
	FROM (SELECT MASV, HO, TEN, MALOP, PASSWORD FROM SINHVIEN) SV, 
		(SELECT MALOP, MAKHOA FROM LOP) L
	WHERE SV.MALOP = L.MALOP 
	AND MASV = @USERNAME AND PASSWORD = CONVERT(VARCHAR(32), HASHBYTES('MD5', @PASSWORD), 2)
END
GO
...
BEGIN
	...
	SELECT LTC.MALTC, SOSVDADANGKY = COUNT(DISTINCT MASV) INTO #DSSOSVDANGKY
...
BEGIN
	SELECT DK.MALTC 
	FROM (SELECT MALTC, NIENKHOA, HOCKY FROM LOPTINCHI WHERE NIENKHOA = @NK AND HOCKY = @HK) LTC,
...
BEGIN
	...
	SELECT MASV INTO #DK
	FROM DANGKY
	WHERE MALTC IN (SELECT MALTC FROM #LTC)
...
END
GO
-- SP_REPORT_DanhSachSinhVienDangKyLopTinChi '2021-2022', 1, 'CTDL', 1
...
BEGIN
	...
	FROM (SELECT MASV, DIEM_CC, DIEM_GK, DIEM_CK FROM DANGKY WHERE MALTC IN (SELECT MALTC FROM #LTC)) DK,
