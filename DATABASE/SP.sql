...
ALTER PROC [dbo].[SP_CHECK_LOPTINCHI]
@NIENKHOA NCHAR(9), @HOCKY INT, @MAMH NCHAR(10), @NHOM INT
AS
BEGIN
	DECLARE @return_value INT = 0
	IF EXISTS(SELECT MALTC FROM dbo.LOPTINCHI WHERE MAMH = @MAMH AND NIENKHOA = @NIENKHOA AND NHOM = @NHOM AND HOCKY = @HOCKY)
		SET @return_value = 1 -- Mã tồn tại ở phân mảnh hiện tại
	ELSE IF EXISTS(SELECT MALTC FROM LINK1.QLDSV_TC.dbo.LOPTINCHI WHERE MAMH = @MAMH AND NIENKHOA = @NIENKHOA AND NHOM = @NHOM AND HOCKY = @HOCKY)
		SET @return_value = 2 -- Mã tồn tại ở phân mảnh khác
	SELECT @return_value
	RETURN
END
...
BEGIN
	DECLARE @return_value INT = 0
	IF EXISTS(SELECT MASV FROM dbo.CT_DONGHOCPHI WHERE MASV = @MASV AND NGAYDONG = @NGAYDONG AND NIENKHOA = @NIENKHOA AND HOCKY = @HOCKY)
		SET @return_value = 1
	SELECT @return_value
	RETURN
END
...
BEGIN
	...
	FROM (SELECT MALTC FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK) LTC
	...
	FROM (SELECT * FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK) LTC,
...
-- Lấy danh sách lớp tín chỉ theo niên khóa - học kỳ (chưa hủy) để nhập điểm và cho sv đăng ký
...
BEGIN
	...
	SELECT LTC.MALTC, SOSVDADANGKY = COUNT(MASV) INTO #DSSOSVDANGKY
	FROM (SELECT MALTC FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) 
			WHERE NIENKHOA = @NK AND HOCKY = @HK AND HUYLOP = 'FALSE') LTC
	LEFT JOIN (SELECT MALTC, MASV FROM DANGKY
			WHERE HUYDANGKY = 'FALSE') DK 
	ON LTC.MALTC = DK.MALTC
	GROUP BY LTC.MALTC

	SELECT LTC.MALTC, LTC.MAMH, TENMH, NHOM, HOTENGV = HO + ' ' + TEN, SOSVTOITHIEU, SOSVDADANGKY
	FROM (SELECT MALTC, MAMH, NHOM, MAGV, SOSVTOITHIEU, HUYLOP 
	FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK AND HUYLOP = 'FALSE') LTC,
		(SELECT MAMH, TENMH FROM MONHOC) MH, 
		(SELECT MAGV, HO, TEN FROM GIANGVIEN) GV, #DSSOSVDANGKY
	WHERE  LTC.MAMH = MH.MAMH AND LTC.MAGV = GV.MAGV AND LTC.MALTC = #DSSOSVDANGKY.MALTC
	ORDER BY TENMH, NHOM
...
-- Lấy danh sách mã môn học đã có điểm trong niên khóa - học kỳ của 1 sinh viên
CREATE PROC [dbo].[SP_LAY_DS_MAMH_DACODIEM]
@NK NCHAR(9), @HK INT, @MASV NCHAR(10)
AS
BEGIN
	SELECT MAMH
	FROM (SELECT MALTC, MAMH FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK) LTC,
		(SELECT MALTC FROM DANGKY WHERE MASV = @MASV AND (DIEM_CC IS NOT NULL OR DIEM_GK IS NOT NULL OR DIEM_CK IS NOT NULL)) DK
	WHERE LTC.MALTC = DK.MALTC
	-- GROUP BY MAMH
END
GO
-- [dbo].[SP_LAY_DS_MAMH_DACODIEM] '2021-2022', 1, 'N15DCCN001'
...
BEGIN
	...
	SELECT LTC.MALTC, SOSVDADANGKY = COUNT(MASV) INTO #DSSOSVDANGKY
	FROM (SELECT MALTC FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK AND HUYLOP = 'FALSE') LTC
	LEFT JOIN (SELECT MALTC, MASV FROM DANGKY WHERE HUYDANGKY = 'FALSE') DK 
	ON LTC.MALTC = DK.MALTC
	GROUP BY LTC.MALTC

	SELECT TENMH, NHOM, HOTENGVGIANG = HO + ' ' + TEN, SOSVTOITHIEU, SOSVDADANGKY
	FROM (SELECT MALTC, MAMH, NHOM, MAGV, SOSVTOITHIEU FROM LOPTINCHI WITH (INDEX=IX_NIENKHOA) WHERE NIENKHOA = @NK AND HOCKY = @HK AND HUYLOP = 'FALSE') LTC,
		(SELECT MAMH, TENMH FROM MONHOC) MH, 
		(SELECT MAGV, HO, TEN FROM GIANGVIEN) GV, #DSSOSVDANGKY
	WHERE  LTC.MAMH = MH.MAMH AND LTC.MAGV = GV.MAGV AND LTC.MALTC = #DSSOSVDANGKY.MALTC
	ORDER BY TENMH, NHOM
...
BEGIN
	...
	DECLARE @MALTC INT = -1
	SELECT @MALTC = MALTC
	FROM LOPTINCHI WITH (INDEX=IX_MAMH)
	WHERE MAMH = @MAMH AND NIENKHOA = @NK AND NHOM = @NHOM AND HOCKY = @HK
...
BEGIN
	...
	FROM (SELECT MASV, DIEM_CC, DIEM_GK, DIEM_CK FROM DANGKY WHERE MALTC = @MALTC) DK,
		(SELECT MASV, HO, TEN FROM SINHVIEN WITH (INDEX=IX_DANGHIHOC) WHERE DANGHIHOC = 'FALSE') SV
...
AS
BEGIN
	SELECT TENMH, DIEM = MAX(DIEM_HM)
	FROM (SELECT MALTC, DIEM_HM = DIEM_CC * 0.1 + DIEM_GK * 0.3 + DIEM_CK * 0.6 FROM DANGKY WHERE MASV = @MASV) DK,
...
BEGIN
	...
	FROM
	 (SELECT MASV, HOCPHI FROM HOCPHI WHERE NIENKHOA = @NK AND HOCKY = @HK) HP
	LEFT JOIN 
		(SELECT MASV, SOTIENDONG FROM CT_DONGHOCPHI WHERE NIENKHOA = @NK AND HOCKY = @HK) CTDHP
	ON HP.MASV = CTDHP.MASV, 
...
BEGIN
	...
	FROM (SELECT MALTC, MASV, DIEM_HM = (0.1 * DIEM_CC + 0.3 * DIEM_GK + 0.6 * DIEM_CK) FROM DANGKY WITH (INDEX=IX_MASV) WHERE MASV IN (SELECT MASV FROM #SV)) DK,
